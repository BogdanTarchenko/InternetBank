...
struct CreateLoanView: View {
    @Bindable var viewModel: CreateLoanViewModel
        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: Layout.formSpacing) {
                    ...
                    TextField(Strings.amountPlaceholder, text: $viewModel.amountText)
                    ...
                    if !viewModel.tariffs.isEmpty {
                        ...
                        Picker("", selection: Binding(
                            get: { viewModel.selectedTariffId ?? viewModel.tariffs.first!.id },
                            set: { viewModel.selectedTariffId = $0 }
                        )) {
                            ForEach(viewModel.tariffs) { t in
                                Text(t.name + " (\(formatRate(t.rate))%)").tag(t.id)
                            }
                        }
                        .pickerStyle(.menu)
                    }
...
private extension CreateLoanView {
    enum Layout {
    ...
    enum Strings {
        static let title = "Выдать кредит"
