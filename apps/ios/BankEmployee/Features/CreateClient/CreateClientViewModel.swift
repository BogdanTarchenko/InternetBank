...
final class CreateClientViewModel {
    ...
    func submit() async {
        ...
        do {
            _ = try await employeeService.createClient(login: login.trimmingCharacters(in: .whitespacesAndNewlines), password: password, displayName: displayName.trimmingCharacters(in: .whitespacesAndNewlines))
